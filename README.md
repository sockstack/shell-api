...
4. 优化测速方案,支持流式和非流式的接口,原版只支持非流式接口测速;
5. 日志中展示渠道名称,并且可以根据渠道名称搜索过滤;
6. 优化测速方式,改为更节省Token的方式;
7. **OpenAI同一个key可以指定组织消耗,不填就走默认组织;**
8. 增强日志内容,原版自动禁用通道后经常没有日志,导致问题很难定位;
9. **所有页面增加每页显示多少条的功能,再也不用10条10条的翻页累死了;**
10. 编辑的时候可以控制显示隐藏秘钥;
11. **用户请求,响应的聊天记录存表以及查询;**
12. 修复原始版本余额不足的用户去请求时候会自动禁用通道的bug;
